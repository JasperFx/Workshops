...
public class categorise_incidents_end_to_end : IntegrationContext
{
    ...
    public async Task categorise_an_incident_happy_path()
    {
        ...
        using var s = theStore.LightweightSession();
        var incident = await s.Events.FetchLatest<Incident>(incidentId);
        incident.Category.ShouldBe(IncidentCategory.Database);
    }
